...
if( isset( $_SESSION['user'] )){
	...
	$updateTitle = $con -> prepare(
		"UPDATE `learn` SET `title`= ? WHERE `id` = ?"
	);
	...
	$updateSidebar = $con -> prepare(
		"UPDATE `learn` SET `sidebar`= ? WHERE `id` = ?"
	);
	$updateSidebar -> bind_param("ss",$sidebar,$id);
	$updateSidebar -> execute();
	
	$updateSidebar = $con -> prepare(
		"UPDATE `learn` SET `c_id`= ? WHERE `id` = ?"
	);
	$updateSidebar -> bind_param("is",$cid,$id);
	$updateSidebar -> execute();
