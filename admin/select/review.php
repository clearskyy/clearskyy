			...
			<?
				foreach($rows as $row){
					...
					echo date("F/j/Y", strtotime($row["date"]));
						echo '</td>
						<td><form action="/admin/edit/edit-review.php" method="post"><input type="hidden" name="title" value="'.$row["title"].'" /><input type="submit" name="submit" id="submit" value="Edit" style="width:100%;" /></form></td>
					</tr>';
				}
			?>
			</table>
