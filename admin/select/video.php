			...
			foreach($rows as $row){
				echo '<tr>
					...
					echo date("F/j/Y", strtotime($row["date"]));
						echo '</td>
						<td><form action="/admin/edit/edit-video.php" method="post"><input type="hidden" name="title" value="'.$row["title"].'" /><input type="submit" name="submit" id="submit" value="Edit" style="width:100%;" /></form></td>
					</tr>';
			}
			?>
		</table>
