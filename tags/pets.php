...
<!doctype html><html><head><title>Pets | clearskyy</title>
<!-- pets listing page for clearskky.net by nate seymour  -->
	...
	<div id="wrap-left">
		<div class="content">
			...
			<?
				...
				$perGET = $con->prepare("SELECT title,date,url FROM `editorials` WHERE `c_id` = 14 ORDER BY date ASC");
				$perGET->execute();
				...
				//query that grabs the rows and columns from editorials that have the proper category id
				$perGET = $con->prepare("SELECT title,date,url FROM `reviews` WHERE `c_id` = 14 ORDER BY date ASC");
				$perGET->execute();
				$perGET->bind_result($title,$date,$url);
				while($perResult = $perGET->fetch()){
					echo '<tr><td><a href="'.$url.'" target="_blank">'.$title.'</a></td><td>'.date('F j, Y', strtotime($date)).'</td></tr>';
