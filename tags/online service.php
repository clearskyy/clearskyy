...
<!doctype html><html><head><title>Online Service | clearskyy</title>
<!-- online service listing page for clearskky.net by nate seymour  -->
...
<div id="nexus">
	<div id="wrap-left">
		<div class="content">
			...
			<h2 align="center">online service</h2>
			...
			<?
				...
				$perGET = $con->prepare("SELECT title,date,url FROM `editorials` WHERE `c_id` = 21 ORDER BY date ASC");
				$perGET->execute();
				...
				//query that grabs the rows and columns from editorials that have the proper category id
				$perGET = $con->prepare("SELECT title,date,url FROM `reviews` WHERE `c_id` = 21 ORDER BY date ASC");
				$perGET->execute();
				$perGET->bind_result($title,$date,$url);
				while($perResult = $perGET->fetch()){
					echo '<tr><td><a href="'.$url.'" target="_blank">'.$title.'</a></td><td>'.date('F j, Y', strtotime($date)).'</td></tr>';
