...
include_once '../crumpocolypse/dbCon.php';
	...
	$ssquery = "SELECT DISTINCT c_id FROM stories";
	$ssresult = $con->query($ssquery);
	
	while($row = $ssresult->fetch_array()){
		$ssrows[] = $row;
	}

?>

<!doctype html><html><head><title>stories | clearskyy</title>
<!-- stories listing page for clearskky.net by nate seymour  -->
...
<div id="wrap-left">
	...
	<div class="content">
		...
		<tr class="th-main">
			...
			$review_query = "SELECT url,title,date FROM stories WHERE c_id = ".$ciDeezy." ORDER BY date DESC";
